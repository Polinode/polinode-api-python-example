...
r = requests.put('https://www.polinode.com/api/v2/networks/' + networkId, auth=(username, password), data = {'name': 'My new network after edits', 'networkJSON': json.dumps(exampleNetworkData)})
network = r.json()
print('Summary of updated network:')
pprint.pprint(network, indent=2)

#Example of retrieving a summary of all networks for a user
r = requests.get('https://www.polinode.com/api/v2/networks', auth=(username, password))
networks = r.json()
...
r = requests.get('https://www.polinode.com/api/v2/networks/' + networkId, auth=(username, password))
network = r.json()
...
r = requests.delete('https://www.polinode.com/api/v2/networks/' + networkId, auth=(username, password))
network = r.json()

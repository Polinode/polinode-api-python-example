...
r = requests.put('https://app.polinode.com/api/v2/networks/' + networkId, auth=(username, password), json = {'name': 'My new network after edits', 'networkJSON': exampleNetworkData})
network = r.json()
print('Summary of updated network:')
pprint.pprint(network, indent=2)

#Example of retrieving a summary of all networks for a user
r = requests.get('https://app.polinode.com/api/v2/networks', auth=(username, password))
networks = r.json()
...
r = requests.get('https://app.polinode.com/api/v2/networks/' + networkId, auth=(username, password))
network = r.json()
...
r = requests.delete('https://app.polinode.com/api/v2/networks/' + networkId, auth=(username, password))
network = r.json()
